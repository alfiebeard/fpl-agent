...
class EmbeddingFilter:
    ...
    def _get_player_positions(self, enriched_data: Dict[str, str]) -> Dict[str, str]:
        ...
        for player_name, enriched_text in enriched_data.items():
            # Extract position from the first line of enriched text
            # Format: "Player Name (POSITION, £price)"
            try:
                position_start = enriched_text.find('(') + 1
                position_end = enriched_text.find(',', position_start)
                if position_start > 0 and position_end > position_start:
                    position = enriched_text[position_start:position_end].strip()
